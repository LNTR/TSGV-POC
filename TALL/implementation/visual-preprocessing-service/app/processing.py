...
def extract_video_frames(
    ...
    while success:
        ...
        if dense:
            resized = cv2.resize(frame, output_frame_size, interpolation=cv2.INTER_LINEAR)
            frames.append(np.expand_dims(resized.astype(np.float32), axis=0))
    ...
    if not frames:
        empty = np.zeros((0, output_frame_size[1], output_frame_size[0], 3), dtype=np.float32)
        return empty, source_fps, source_frame_count

    return np.concatenate(frames).astype(np.float32), source_fps, source_frame_count
...
def run_preprocess_job(
    ...
    if encoder != "c3d":
        raise ValueError("TALL visual preprocessing only supports encoder='c3d'.")
    ...
    sampling_strategy = "all_frames_for_c3d" if encoder == "c3d" else "sparse_frames"
    ...
    if existing_metadata is not None and os.path.exists(output_frames_path):
        if (
            ...
            and existing_metadata.get("sampling_strategy") == sampling_strategy
        ):
            return {
                ...
                "source_fps": float(existing_metadata.get("source_fps", 0.0)),
                "video_hash": video_hash,
                "cache_hit": True,
        ...
        if isinstance(cached_frames_uri, str):
            ...
            if os.path.exists(cached_frames_path):
                ...
                metadata = {
                    ...
                    "sampling_strategy": sampling_strategy,
                    "video_hash": video_hash,
                    "source_fps": float(cached.get("source_fps", 0.0)),
                    "num_source_frames": int(cached.get("num_source_frames", 0)),
                    "preprocess_cache_key": cache_key,
                    "cache_source": "redis",
                }
    ...
    frames, source_fps, source_frame_count = extract_video_frames(
...
def forward_to_feature_service(
    ...
    response = requests.post(
        ...
        json={
            ...
            "clip_stride_sec": sample_every_sec,
            "normalize": normalize,
